...
public partial class PrefixedUlidGenerator : IIncrementalGenerator {
    ...
    static void Emit(SourceProductionContext context, GeneratorAttributeSyntaxContext source) {
        ...
        var code = $$"""
            ...
            partial record {{typeSymbol.Name}} : Hadron.Identifier.PrefixedUlid {
                public const int TotalLength = {{(prefix?.Length ?? 0) + 1 + 26}};
                public const string Prefix = "{{prefix}}";
                public const string Separator = "{{separator}}";

                public static {{typeSymbol.Name}} Default {get; } = Parse($"{{prefix}}{{separator}}{default(Hadron.Identifier.Ulid)}");

                public override string PrefixWithSeparator => "{{prefix}}{{separator}}";
 
                public static {{typeSymbol.Name}} CreateNew() => Hadron.Identifier.PrefixedUlid.NewPrefixedUlid<{{typeSymbol.Name}}>();

                public static {{typeSymbol.Name}} Parse(string id) {
                    return Hadron.Identifier.PrefixedUlid.Parse<{{typeSymbol.Name}}>(id);
                }
